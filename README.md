...
It is **warm neutrals, dark by default, with one selectable accent and no
emoji**; the reasoning is in
[docs/adr/0015-qdos-design-language.md](docs/adr/0015-qdos-design-language.md)
and [docs/adr/0028-ruhe-design-system.md](docs/adr/0028-ruhe-design-system.md).
...
| API | Register (optionally through an invite link)/sign in/sign out/session/delete the account/ask for a reset link/set a new password with it; goals with their schedules and windows, what is due today, the archive of the ones that stopped, pause and objection, feed and kudos, friends with search and requests, chats including starting, muting and leaving them, profile (read and update) and settings; images with an upload, a session-checked read and a delete; your own proof photographs; the daily challenge with today's room, a contribution, a withdrawal, reactions and your own archive; blocking, unblocking and the list of who you blocked; reporting a person, a photograph or a contribution; your invite code and a way to replace it, and for somebody else's: whose it is and accepting it; the bell, deleting a line from it or everything up to a given moment, and every badge count in one read; the push key, subscribing this browser and unsubscribing it; a live connection at `/api/live` that tells an open app what changed and hands it what would otherwise have been a push; `GET /health`, OpenAPI document, Problem Details for every error. Deliberately **no ranking endpoint** — see [docs/adr/0015](docs/adr/0015-qdos-design-language.md) |
...
| Proof and vote | A window is closed by a photograph its friends believe, not by its owner saying so. More than a third doubting refuses it, at least two doubters are needed, one retry is allowed, and twelve hours later the votes cast decide it whether or not anybody has the app open. Confirmations are named; doubts never are. Friends' photographs wait on the start screen as a stack swiped right to confirm and left to doubt; a vote can be changed in the goal's conversation until the photograph is decided, and every photograph opens full screen — see [docs/adr/0018-proof-and-vote.md](docs/adr/0018-proof-and-vote.md) and [docs/adr/0030](docs/adr/0030-swipe-vote-and-full-screen-photos.md) |
...
| Leaving | An account can be deleted: the person, their goals and windows with the conversation of each, their pictures and their direct chats go, their messages leave the groups and friends' goals they were in, and the password is asked for again. It is the only irreversible action in q2 |
...
| Arriving | An invite link — 96 random bits, replaceable, never the handle — opens a page naming its sender and makes whoever follows it a friend: at registration, or with one tap for somebody who already has an account ([ADR 0033](docs/adr/0033-invite-links-for-new-and-existing-accounts.md)). That plus the daily challenge is what a new account has on its first day |
...
| Frontend | Sign-in, registration and password reset, the six screens plus goal detail, chat thread and settings; a global route guard; loading, empty, error and not-found states; German and English; light and dark with four device-local accent palettes ([ADR 0028](docs/adr/0028-ruhe-design-system.md)); mobile-first, developed and tested at phone width, keyboard accessible |
